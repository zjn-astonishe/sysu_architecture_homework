...
#error "Please define a way to initialize a memory block."
	...
	if (known_id>=0) {
		for (i=0 ; i<default_num_contexts; i++) {
			...
			if ((results[i].execs & ID_LIST) &&
				...
				ee_printf("[%d]ERROR! list crc %d - should be %d\n",i,results[i].crclist,list_known_crc[known_id]);
				results[i].err++;
			}
			if ((results[i].execs & ID_MATRIX) &&
				(results[i].crcmatrix!=matrix_known_crc[known_id])) {
				ee_printf("[%d]ERROR! matrix crc %d - should be %d\n",i,results[i].crcmatrix,matrix_known_crc[known_id]);
				results[i].err++;
			}
			if ((results[i].execs & ID_STATE) &&
				(results[i].crcstate!=state_known_crc[known_id])) {
				ee_printf("[%d]ERROR! state crc %d - should be %d\n",i,results[i].crcstate,state_known_crc[known_id]);
				results[i].err++;
			}
	...
	if (results[0].execs & ID_LIST)
		for (i=0 ; i<default_num_contexts; i++)
			ee_printf("[%d]crclist       : %d\n",i,results[i].crclist);
	if (results[0].execs & ID_MATRIX)
		for (i=0 ; i<default_num_contexts; i++)
			ee_printf("[%d]crcmatrix     : %d\n",i,results[i].crcmatrix);
	if (results[0].execs & ID_STATE)
		for (i=0 ; i<default_num_contexts; i++)
			ee_printf("[%d]crcstate      : %d\n",i,results[i].crcstate);
	for (i=0 ; i<default_num_contexts; i++)
		ee_printf("[%d]crcfinal      : %d\n",i,results[i].crc);
	if (total_errors==0) {
